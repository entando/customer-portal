...
import { apiPortalUsersGet, apiPortalUsersDelete } from 'api/portalUsers';
import { reducer, initialState } from 'state/portalUser.reducer';
import { ADD_FILTER, UPDATE_FILTER, DELETE_FILTER, CLEAR_FILTERS } from 'state/filter.types';
import { DELETE, ERROR_FETCH, CLEAR_ERRORS, READ_ALL, CLEAR_ITEMS } from 'state/portalUser.types';
...
class PortalUserTableContainer extends Component {
  constructor(props) {
    super(props);
  ...
  async fetchData() {
    ...
    if (authenticated) {
      try {
        ...
        const { portalUsers, headers } = await apiPortalUsersGet(serviceUrl, requestParameters);
        const portalUserCount = (headers && headers['X-Total-Count']) || 0;

        this.dispatch({
          type: READ_ALL,
          payload: {
            items: paginationMode === 'infinite-scroll' ? [...items, ...portalUsers] : portalUsers,
  ...
  async handleDelete(item) {
    ...
    if (authenticated) {
      try {
        await apiPortalUsersDelete(serviceUrl, item.id);
        onDelete(item);
        ...
        <AuthenticatedView keycloak={keycloak}>
          ...
          <PaginationWrapper items={items} paginationMode={paginationMode} count={count}>
            <div className={classes.tableWrapper}>
              <PortalUserTable items={items} onSelect={onSelect} Actions={Actions} />
            </div>
          </PaginationWrapper>
        </AuthenticatedView>
        <Notification
...
PortalUserTableContainer.propTypes = {
...
PortalUserTableContainer.defaultProps = {
  onAdd: () => {},
  onDelete: () => {},
  onError: () => {},
  onSelect: () => {},
...
export default withKeycloak(
  withStyles(styles)(
    withTranslation(undefined, { withRef: true })(withPagination(PortalUserTableContainer))
  )
);
