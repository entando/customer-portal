...
import { apiCustomersGet, apiCustomersDelete } from 'api/customers';
import { reducer, initialState } from 'state/customer.reducer';
import { ADD_FILTER, UPDATE_FILTER, DELETE_FILTER, CLEAR_FILTERS } from 'state/filter.types';
import { DELETE, ERROR_FETCH, CLEAR_ERRORS, READ_ALL, CLEAR_ITEMS } from 'state/customer.types';
...
class CustomerTableContainer extends Component {
  ...
  async fetchData() {
    ...
    if (authenticated) {
      try {
        ...
        const { customers, headers } = await apiCustomersGet(serviceUrl, requestParameters);
        const customerCount = (headers && headers['X-Total-Count']) || 0;

        this.dispatch({
          type: READ_ALL,
          payload: {
            items: paginationMode === 'infinite-scroll' ? [...items, ...customers] : customers,
            count: customerCount,
          },
        });
      } catch (err) {
  ...
  async handleDelete(item) {
    ...
    if (authenticated) {
      try {
        await apiCustomersDelete(serviceUrl, item.id);
        onDelete(item);
        ...
        <AuthenticatedView keycloak={keycloak}>
          ...
          <PaginationWrapper items={items} paginationMode={paginationMode} count={count}>
            <div className={classes.tableWrapper}>
              <CustomerTable items={items} onSelect={onSelect} Actions={Actions} />
            </div>
          </PaginationWrapper>
        </AuthenticatedView>
        <Notification
...
CustomerTableContainer.defaultProps = {
  onAdd: () => {},
  onDelete: () => {},
  onError: () => {},
  onSelect: () => {},
...
export default withKeycloak(
  withStyles(styles)(
    withTranslation(undefined, { withRef: true })(withPagination(CustomerTableContainer))
  )
);
