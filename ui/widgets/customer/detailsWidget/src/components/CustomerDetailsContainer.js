...
class CustomerDetailsContainer extends React.Component {
  ...
  async fetchData() {
    ...
    if (authenticated && id) {
      try {
        const customer = await apiCustomerGet(serviceUrl, id);
        this.setState(() => ({
          customer,
          loading: false,
        }));
  ...
  render() {
    const { customer, notificationStatus, notificationMessage, loading } = this.state;
...
export default withKeycloak(withTranslation()(CustomerDetailsContainer));
