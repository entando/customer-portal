...
import { apiProjectsGet, apiProjectsDelete } from 'api/projects';
import { reducer, initialState } from 'state/project.reducer';
import { ADD_FILTER, UPDATE_FILTER, DELETE_FILTER, CLEAR_FILTERS } from 'state/filter.types';
import { DELETE, ERROR_FETCH, CLEAR_ERRORS, READ_ALL, CLEAR_ITEMS } from 'state/project.types';
...
class ProjectTableContainer extends Component {
  constructor(props) {
    super(props);
  ...
  async fetchData() {
    ...
    if (authenticated) {
      try {
        ...
        const { projects, headers } = await apiProjectsGet(serviceUrl, requestParameters);
        const projectCount = (headers && headers['X-Total-Count']) || 0;

        this.dispatch({
          type: READ_ALL,
          payload: {
            items: paginationMode === 'infinite-scroll' ? [...items, ...projects] : projects,
            count: projectCount,
          },
        });
      } catch (err) {
  ...
  async handleDelete(item) {
    ...
    if (authenticated) {
      try {
        await apiProjectsDelete(serviceUrl, item.id);
        onDelete(item);
        ...
        <AuthenticatedView keycloak={keycloak}>
          ...
          <PaginationWrapper items={items} paginationMode={paginationMode} count={count}>
            <div className={classes.tableWrapper}>
              <ProjectTable items={items} onSelect={onSelect} Actions={Actions} />
            </div>
          </PaginationWrapper>
        </AuthenticatedView>
        <Notification
...
ProjectTableContainer.propTypes = {
...
ProjectTableContainer.defaultProps = {
  onAdd: () => {},
  onDelete: () => {},
  onError: () => {},
  onSelect: () => {},
...
export default withKeycloak(
  withStyles(styles)(
    withTranslation(undefined, { withRef: true })(withPagination(ProjectTableContainer))
  )
);
