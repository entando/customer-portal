...
class ProjectAddFormContainer extends PureComponent {
  constructor(props) {
    super(props);
  ...
  async handleSubmit(project) {
    ...
    if (authenticated) {
      try {
        const createdProject = await apiProjectPost(serviceUrl, project);
    ...
    return (
      <>
        ...
        <AuthenticatedView keycloak={keycloak}>
          <ProjectForm onSubmit={this.handleSubmit} onCancelEditing={onCancelEditing} />
        </AuthenticatedView>
        <Notification
...
ProjectAddFormContainer.propTypes = {
...
ProjectAddFormContainer.defaultProps = {
  onError: () => {},
  onCancelEditing: () => {},
  onCreate: () => {},
  serviceUrl: '',
...
export default withKeycloak(withTranslation()(ProjectAddFormContainer));
