...
import {
  apiProjectSubscriptionsGet,
  apiProjectSubscriptionsDelete,
} from 'api/projectSubscriptions';
import { reducer, initialState } from 'state/projectSubscription.reducer';
...
import {
  ...
  READ_ALL,
  CLEAR_ITEMS,
} from 'state/projectSubscription.types';
...
class ProjectSubscriptionTableContainer extends Component {
  constructor(props) {
    super(props);
  ...
  async fetchData() {
    ...
    if (authenticated) {
      try {
        ...
        const { projectSubscriptions, headers } = await apiProjectSubscriptionsGet(
          serviceUrl,
          requestParameters
        );
        const projectSubscriptionCount = (headers && headers['X-Total-Count']) || 0;
          ...
          payload: {
            items:
              paginationMode === 'infinite-scroll'
                ? [...items, ...projectSubscriptions]
                : projectSubscriptions,
            count: projectSubscriptionCount,
          },
        });
      } catch (err) {
  ...
  async handleDelete(item) {
    ...
    if (authenticated) {
      try {
        await apiProjectSubscriptionsDelete(serviceUrl, item.id);
        onDelete(item);
        ...
        <AuthenticatedView keycloak={keycloak}>
          ...
          <PaginationWrapper items={items} paginationMode={paginationMode} count={count}>
            <div className={classes.tableWrapper}>
              <ProjectSubscriptionTable items={items} onSelect={onSelect} Actions={Actions} />
            </div>
          </PaginationWrapper>
        </AuthenticatedView>
        <Notification
...
ProjectSubscriptionTableContainer.propTypes = {
...
ProjectSubscriptionTableContainer.defaultProps = {
  onAdd: () => {},
  onDelete: () => {},
  onError: () => {},
  onSelect: () => {},
...
export default withKeycloak(
  withStyles(styles)(
    withTranslation(undefined, { withRef: true })(withPagination(ProjectSubscriptionTableContainer))
  )
);
