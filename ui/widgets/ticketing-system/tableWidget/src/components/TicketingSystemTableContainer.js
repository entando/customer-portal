...
import { apiTicketingSystemsGet, apiTicketingSystemsDelete } from 'api/ticketingSystems';
import { reducer, initialState } from 'state/ticketingSystem.reducer';
...
import {
  ...
  READ_ALL,
  CLEAR_ITEMS,
} from 'state/ticketingSystem.types';
...
class TicketingSystemTableContainer extends Component {
  constructor(props) {
    super(props);
  ...
  async fetchData() {
    ...
    if (authenticated) {
      try {
        ...
        const { ticketingSystems, headers } = await apiTicketingSystemsGet(
          serviceUrl,
          requestParameters
        );
        const ticketingSystemCount = (headers && headers['X-Total-Count']) || 0;
          ...
          payload: {
            items:
              paginationMode === 'infinite-scroll'
                ? [...items, ...ticketingSystems]
                : ticketingSystems,
            count: ticketingSystemCount,
          },
        });
      } catch (err) {
  ...
  async handleDelete(item) {
    ...
    if (authenticated) {
      try {
        await apiTicketingSystemsDelete(serviceUrl, item.id);
        onDelete(item);
        ...
        <AuthenticatedView keycloak={keycloak}>
          ...
          <PaginationWrapper items={items} paginationMode={paginationMode} count={count}>
            <div className={classes.tableWrapper}>
              <TicketingSystemTable items={items} onSelect={onSelect} Actions={Actions} />
            </div>
          </PaginationWrapper>
        </AuthenticatedView>
        <Notification
...
TicketingSystemTableContainer.propTypes = {
...
TicketingSystemTableContainer.defaultProps = {
  onAdd: () => {},
  onDelete: () => {},
  onError: () => {},
  onSelect: () => {},
...
export default withKeycloak(
  withStyles(styles)(
    withTranslation(undefined, { withRef: true })(withPagination(TicketingSystemTableContainer))
  )
);
