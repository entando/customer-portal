...
import { apiEntandoVersionsGet, apiEntandoVersionsDelete } from 'api/entandoVersions';
import { reducer, initialState } from 'state/entandoVersion.reducer';
...
import {
  ...
  READ_ALL,
  CLEAR_ITEMS,
} from 'state/entandoVersion.types';
...
class EntandoVersionTableContainer extends Component {
  constructor(props) {
    super(props);
  ...
  async fetchData() {
    ...
    if (authenticated) {
      try {
        ...
        const { entandoVersions, headers } = await apiEntandoVersionsGet(
          serviceUrl,
          requestParameters
        );
        const entandoVersionCount = (headers && headers['X-Total-Count']) || 0;
          ...
          payload: {
            items:
              paginationMode === 'infinite-scroll'
                ? [...items, ...entandoVersions]
                : entandoVersions,
            count: entandoVersionCount,
          },
        });
      } catch (err) {
  ...
  async handleDelete(item) {
    ...
    if (authenticated) {
      try {
        await apiEntandoVersionsDelete(serviceUrl, item.id);
        onDelete(item);
    ...
    const Actions = ({ item }) =>
      onDelete ? (
        <ConfirmationDialogTrigger
          ...
          dialog={{
            title: t('entities.entandoVersion.deleteDialog.title'),
            description: t('entities.entandoVersion.deleteDialog.description'),
        ...
        <AuthenticatedView keycloak={keycloak}>
          ...
          <PaginationWrapper items={items} paginationMode={paginationMode} count={count}>
            <div className={classes.tableWrapper}>
              <EntandoVersionTable items={items} onSelect={onSelect} Actions={Actions} />
            </div>
          </PaginationWrapper>
        </AuthenticatedView>
        <Notification
...
EntandoVersionTableContainer.propTypes = {
...
EntandoVersionTableContainer.defaultProps = {
  onAdd: () => {},
  onDelete: () => {},
  onError: () => {},
  onSelect: () => {},
...
export default withKeycloak(
  withStyles(styles)(
    withTranslation(undefined, { withRef: true })(withPagination(EntandoVersionTableContainer))
  )
);
